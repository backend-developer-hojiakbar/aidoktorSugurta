...
const ComprehensiveAnalysisMode = ({ onBack }) => {
  ...
  const fetchAd = useCallback(async (step: string) => {
    try {
      ...
      if (Array.isArray(response.data) && response.data.length > 0) {
        const ad = response.data.find(ad => ad.category === 'analysis' && ad.is_active);
        if (ad) {
      ...
      console.error(`Failed to fetch ad for ${step} step:`, error.response ? error.response.data : error.message);
  ...
  useEffect(() => {
    ...
    fetchAd(currentStep); // Har bir qadamda reklama yuklash
  }, [getInitialQuestions, getQuestionPrompts, currentStep, fetchAd]);
  ...
  const handleFileTypeChange = (id: string, type: string) => {
  ...
  const fileTypeOptions = Object.entries(FileType).map(([key, value]) => ({ value, label: t_noDynamic(value) || value }));
            ...
            {adData ? (
              <a href={adData.link} target="_blank" rel="noopener noreferrer">
                <img
                  src={adData.image}
                  alt="Advertisement"
                  className="max-w-md mx-auto h-auto mb-4"
              ...
              <p className="text-sm text-red-500 text-center mb-4" role="alert">{adError}</p>
            )}
            {fileErrors.length > 0 && (
            ...
            {adData ? (
              <a href={adData.link} target="_blank" rel="noopener noreferrer">
                <img
                  src={adData.image}
                  alt="Advertisement"
                  className="max-w-md mx-auto h-auto mt-2"
                  style={{ maxHeight: '100px' }}
                  onError={(e) => {
                    console.error('Image failed to load:', e);
                    setAdError(t_noDynamic('adImageFailed'));
              ...
              <p className="text-sm text-red-500 text-center mt-2" role="alert">{adError}</p>
            )}
