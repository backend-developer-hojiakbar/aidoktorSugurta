  ...
  const fetchAd = useCallback(async () => {
    try {
      const response = await axios.get('https://aidoktor.pythonanywhere.com/advertisements/', {
        params: {
          category: 'online_nurse',
          size: '468x60',
          is_active: true,
        },
      });
      if (Array.isArray(response.data) && response.data.length > 0) {
        const ad = response.data.find(ad => ad.category === 'online_nurse' && ad.is_active);
        if (ad) {
      ...
      console.error('Failed to fetch ad for OnlineHamshira:', error.response ? error.response.data : error.message);
