...
const OnLabMode = ({ onBack }) => {
  ...
  const fetchAd = useCallback(async () => {
    try {
      ...
      if (Array.isArray(response.data) && response.data.length > 0) {
        const ad = response.data.find(ad => ad.category === 'onlab' && ad.is_active);
        if (ad) {
      ...
      console.error('Failed to fetch ad for OnLab:', error.response ? error.response.data : error.message);
  ...
  return (
    <>
      <div className="max-w-3xl mx-auto p-6 md:p-10 shadow-2xl rounded-lg bg-gradient-to-b from-orange-50/80 to-amber-50/70 backdrop-blur-md text-slate-800">
        <h2 className="text-3xl font-bold text-center mb-2 uppercase text-amber-700">{t_noDynamic('onLabModeTitle')}</h2>
        <h3 className="text-xl font-semibold text-center mb-6 uppercase text-slate-700">{t_noDynamic('onLabModeInfoTitle')}</h3>
        ...
        <p className="mb-8 text-md leading-relaxed text-center text-slate-600">
          {t_noDynamic('onLabModeDescription')}
        </p>

        <div className="mb-10 text-center">
          <p className="text-lg mb-2 text-slate-600">{t_noDynamic('onLabCallToAction')}</p>
          <a 
        ...
        <div className="mb-8 text-center">
          <h4 className="text-xl font-semibold mb-4 text-slate-700">{t_noDynamic('onLabDownloadTitle')}</h4>
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mb-6">
            <a href={playStoreLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center bg-slate-100/80 hover:bg-slate-200/90 text-slate-700 font-medium py-3 px-5 rounded-lg transition-colors w-full sm:w-auto border border-slate-300/80">
              ...
              {t_noDynamic('onLabPlayStoreLinkText')}
            </a>
            <a href={appStoreLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center bg-slate-100/80 hover:bg-slate-200/90 text-slate-700 font-medium py-3 px-5 rounded-lg transition-colors w-full sm:w-auto border border-slate-300/80">
              ...
              {t_noDynamic('onLabAppStoreLinkText')}
            </a>
          </div>
          <p className="text-sm text-slate-500 mb-2">{t_noDynamic('onLabQrCodeText')}:</p>
